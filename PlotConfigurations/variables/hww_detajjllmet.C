...
float detajjllmet(float jet1_pt, float jet1_eta, float jet1_phi,
                  ...
                  float met_pt, float met_phi)         
{
  ...
  return fabs((jet1+jet2).Eta()-(lepton1+lepton2+met).Eta());
}


float hww_detajjllmet(int entry)
{
  TTreeReader fReader;
  ...
  return detajjllmet(CleanJet_pt[0], CleanJet_eta[0], CleanJet_phi[0],
