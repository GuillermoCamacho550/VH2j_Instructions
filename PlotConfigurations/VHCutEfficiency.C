const TString path1 = "plotStorage/SequentialCuts/Individual_Final/";
const TString path2 = "plotStorage/SequentialCuts/Stacking_Final/";

void VHCutEfficiency()
{
  TFile* file_ZH0   = new TFile(path1 + "plots_WW_ALL_ZH_hww.root", "read");
  ...
  TH1D* events_ZH0   = (TH1D*)file_ZH0  ->Get("VH_2j_emu_0/events/histo_ZH_hww");
  ...
  float  Base;
  Base=events_ZH0->Integral(-1,-1)+events_ggZH0->Integral(-1,-1)+events_WH0->Integral(-1,-1);
  ...
  for (Int_t i=1; i<=11; i++) {
   ...
   TFile* file_ZH   = new TFile(path1 + "plots_WW_ALL_ZH_hww.root", "read");
   ...
   TH1D* events_ZH   = (TH1D*)file_ZH  ->Get("VH_2j_emu_" + number + "/events/histo_ZH_hww");
   ...
   float  Events;
   Events = events_ZH->Integral(-1,-1)+events_ggZH->Integral(-1,-1)+events_WH->Integral(-1,-1);
  ...
  for (Int_t i=1; i<=11; i++) {
   ...
   TFile* file_ZH   = new TFile(path2 + "plots_WW_ALL_ZH_hww.root", "read");
