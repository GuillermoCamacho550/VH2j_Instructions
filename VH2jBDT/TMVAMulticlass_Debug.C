...
void TMVAMulticlass_Debug( TString myMethodList = "" )
{
   ...
   TString workdir = "/eos/cms/store/group/phys_higgs/cmshww/amassiro/HWWNano/Fall2017_nAOD_v1_Full2017v2/MCl1loose2017v2__MCCorr2017__btagPerEvent__l2loose__l2tightOR2017"; //workdir1 2017
...
for (UInt_t k=0; k<29; k++)  {
     if (k < 20) VBF -> Add(Form("%s/nanoLatino_HZJ_HToWW_M120__part%d.root", workdir.Data(), k));
     //if (k < 20) VBF -> Add(Form("%s/nanoLatino_HZJ_HToWW_M120__part%d.root", workdir.Data(), k));
     //if (k < 20) VBF -> Add(Form("%s/nanoLatino_HZJ_HToWW_M120__part%d.root", workdir.Data(), k));
     //if (k < 19) VBF -> Add(Form("%s/nanoLatino_GluGluZH_HToWW_M125__part%d.root", workdir.Data(), k));
     //    if (k < 21) VBF -> Add(Form("%s/nanoLatino_HWplusJ_HToWW_M125__part%d.root",  workdir.Data(), k));
     //if (k < 29) VBF -> Add(Form("%s/nanoLatino_HWminusJ_HToWW_M125__part%d.root", workdir.Data(), k));
}
...
int main( int argc, char** argv )
{
   ...
   TMVAMulticlass_Debug(methodList);
   return 0;
}
