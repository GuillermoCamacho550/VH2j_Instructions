...
if "H_htt"  in signals: signals.remove("H_htt") #Remove to avoid error in Line 115 
...
for channel in DC.exp:
 ...
 # elif 'vh' in channel: c='2-jet VH-tagged'
  elif 'VH_2j_emu' in channel: c='2-jet VH-tagged'
  ...
  for s in signals :
    if s in DC.exp[channel].keys(): # possible that some backgrounds/signals appear only in some channels
      if s == 'ggH_htt':
        ...
        overallSignalRate[c][s] +=  DC.exp[channel][s] #Most signals come from here
...
##  if overallTotalSignal[c] == 0: continue
  ...
  for s in overallSignalRate[c]:
    ...
    histos[s].SetFillColor(color[j])
    histos[s].SetLineColor(color[j])
